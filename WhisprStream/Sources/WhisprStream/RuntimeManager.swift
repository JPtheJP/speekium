...
final class RuntimeManager: ObservableObject {
    ...
    var executableURL: URL? {
        ...
        if let override = ProcessInfo.processInfo.environment["WHISPR_PYTHON"], isExecutable(override) {
    ...
    var isUsingDevelopmentEngine: Bool {
        ...
        if let override = ProcessInfo.processInfo.environment["WHISPR_PYTHON"] {
    ...
    func refresh() {
        ...
        if ProcessInfo.processInfo.environment["WHISPR_PYTHON"].map(isExecutable) == true {
            phase = .ready
            return
        }
    ...
    private var runtimeConfigurationVersion: String? {
        let value = ProcessInfo.processInfo.environment["WHISPR_RUNTIME_VERSION"]
            ?? Bundle.main.object(forInfoDictionaryKey: "WhisprRuntimeVersion") as? String
    ...
    private var runtimeDirectory: URL {
        if let override = ProcessInfo.processInfo.environment["WHISPR_RUNTIME_DIRECTORY"], !override.isEmpty {
