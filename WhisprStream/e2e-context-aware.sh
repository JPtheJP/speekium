...
cd "$(dirname "$0")"
PROJECT_ROOT="$(cd .. && pwd)"
APP="${WHISPR_E2E_APP:-$PROJECT_ROOT/WhisprStream.app}"
EXPECTED="Hello this testing works. "
E2E_TMP="$(mktemp -d /tmp/whispr-context-e2e.XXXXXX)"
...
plutil -insert CFBundleIdentifier -string dev.local.whisprstream-context-e2e "$HELPER_PLIST"
...
open -g -n "$APP" --args \
    --whispr-e2e-context-aware-capitalization \
    "--whispr-e2e-status=$STATUS" \
    "--whispr-e2e-target-pid=$HELPER_PID"
