...
private enum SignerError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .usage:
            return "usage: WhisprStreamUpdateSigner generate|public-key|export <path>|import <path>|sign <archive> <signature>|verify <public-key> <archive> <signature>"
        case let .keychain(status):
            return "Keychain operation failed (\(status))."
        case .noKey:
            return "No WhisprStream update-signing key exists in this Keychain."
        case .keyAlreadyExists:
...
private enum KeyStore {
    static let service = "com.leoleo.whisprstream.update-signing"
    ...
    // signed with the stable WhisprStream identity so its Keychain access
    // survives rebuilds.
...
@main
private enum WhisprStreamUpdateSigner {
    static func main() {
        do {
            try run()
        } catch {
