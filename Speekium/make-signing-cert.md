...
3. Fill in:
   - **Name:** `Speekium Self-Signed`  ← must match exactly, `build.sh` looks for it
...
**System Settings → Privacy & Security → Accessibility** → select Speekium →
...
`~/Library/Logs/Speekium.log` records the state at every launch:

```
[speekium] launch: accessibility=granted
```
