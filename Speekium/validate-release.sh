...
APP_DIR="$VALIDATION_TMP/app/Speekium.app"
[ -d "$APP_DIR" ] && [ ! -L "$APP_DIR" ] || {
    echo "error: app archive must contain Speekium.app at its top level" >&2
    exit 1
}

BIN="$APP_DIR/Contents/MacOS/Speekium"
UPDATE_INSTALLER="$APP_DIR/Contents/Helpers/SpeekiumUpdateInstaller"
...
RUNTIME_VERSION="$(required_plist SpeekiumRuntimeVersion)"
RUNTIME_URL="$(required_plist SpeekiumRuntimeURL)"
RUNTIME_SHA256="$(required_plist SpeekiumRuntimeSHA256)"
RUNTIME_CONTENT_SHA256="$(required_plist SpeekiumRuntimeContentSHA256)"
ARCHIVE_BYTES="$(required_plist SpeekiumRuntimeArchiveBytes)"
INSTALLED_BYTES="$(required_plist SpeekiumRuntimeInstalledBytes)"
UPDATE_REPOSITORY="$(required_plist SpeekiumUpdateRepository)"
UPDATE_PUBLIC_KEY="$(required_plist SpeekiumUpdatePublicKey)"
OPTIONAL_MODELS_ENABLED="$(required_plist SpeekiumOptionalModelsEnabled)"
