...
HELPER="$SCRIPT_DIR/.build/debug/SpeekiumUpdateInstaller"
SYSTEM_TMP="${TMPDIR:-/tmp}"
SYSTEM_TMP="${SYSTEM_TMP%/}"
TEST_ROOT="$(mktemp -d /tmp/speekium-updater-e2e.XXXXXX)"
CLEANUP_ROOTS=()
...
cleanup() {
    ...
    for path in "${CLEANUP_ROOTS[@]}"; do
        case "$path" in
            "$TEST_ROOT"|"$TEST_ROOT"/*|"$SYSTEM_TMP"/Speekium-installer-*)
                rm -rf -- "$path"
                ;;
        esac
    done
...
create_app() {
    local app="$1"
    local version="$2"
    local executable_name="${3:-Speekium}"
    local launch_behavior="${4:-healthy}"
    local plist="$app/Contents/Info.plist"
    mkdir -p "$app/Contents/MacOS"
    plutil -create xml1 "$plist"
    plutil -insert CFBundleIdentifier -string com.jpthejp.speekium "$plist"
    ...
    if [ "$executable_name" = "Speekium" ]; then
        if [ "$launch_behavior" = "healthy" ]; then
            cp "$HEALTHY_EXECUTABLE" "$app/Contents/MacOS/Speekium"
        else
            cp /usr/bin/true "$app/Contents/MacOS/Speekium"
        fi
    fi
}

new_helper_root() {
    NEW_HELPER_ROOT="$(mktemp -d "$SYSTEM_TMP/Speekium-installer-XXXXXX")"
...
swift build --package-path "$SCRIPT_DIR" --product SpeekiumUpdateInstaller >/dev/null
...
SUCCESS_TARGET="$SUCCESS_ROOT/Speekium.app"
SUCCESS_STAGED="$SUCCESS_ROOT/.Speekium.update-success.app"
...
assert_equal 0 "$(find "$SUCCESS_ROOT" -maxdepth 1 -name '.Speekium.update-backup-*.app' | wc -l | tr -d ' ')" \
...
ROLLBACK_TARGET="$ROLLBACK_ROOT/Speekium.app"
ROLLBACK_STAGED="$ROLLBACK_ROOT/.Speekium.update-rollback.app"
...
assert_equal Speekium "$(bundle_value "$ROLLBACK_TARGET" CFBundleExecutable)" \
    "rollback did not restore the previous executable"
assert_equal 0 "$(find "$ROLLBACK_ROOT" -maxdepth 1 -name '.Speekium.update-*.app' | wc -l | tr -d ' ')" \
...
HEALTH_TARGET="$HEALTH_ROOT/Speekium.app"
HEALTH_STAGED="$HEALTH_ROOT/.Speekium.update-health-timeout.app"
mkdir -p "$HEALTH_ROOT"
create_app "$HEALTH_TARGET" 1.0.1
create_app "$HEALTH_STAGED" 9.9.9 Speekium no-health
...
set +e
SPEEKIUM_UPDATE_HEALTH_TIMEOUT=3 run_helper \
...
SAFETY_TARGET="$SAFETY_ROOT/Speekium.app"
SAFETY_STAGED="$SAFETY_ROOT/Speekium.update-unsafe.app"
mkdir -p "$SAFETY_ROOT"
...
echo "The installed Speekium app and /Applications were not touched."
