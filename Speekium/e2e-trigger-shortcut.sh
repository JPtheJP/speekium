...
cd "$(dirname "$0")"
PROJECT_ROOT="$(cd .. && pwd)"
APP="$(printenv SPEEKIUM_E2E_APP || true)"
if [ -z "$APP" ]; then APP="$PROJECT_ROOT/Speekium.app"; fi
EXPECTED_VERSION="$(printenv SPEEKIUM_E2E_VERSION || true)"
if [ -z "$EXPECTED_VERSION" ]; then EXPECTED_VERSION=1.1.0; fi
EXPECTED_BUILD="$(printenv SPEEKIUM_E2E_BUILD_NUMBER || true)"
if [ -z "$EXPECTED_BUILD" ]; then EXPECTED_BUILD=3; fi
SKIP_BUILD="$(printenv SPEEKIUM_E2E_SKIP_BUILD || true)"
if [ -z "$SKIP_BUILD" ]; then SKIP_BUILD=0; fi
E2E_TMP="$(mktemp -d /tmp/speekium-trigger-shortcut-e2e.XXXXXX)"
...
if [ "$SKIP_BUILD" != "1" ]; then
    echo "▸ building Speekium $EXPECTED_VERSION"
...
"$APP/Contents/MacOS/Speekium" \
    --speekium-e2e-trigger-shortcut \
    "--speekium-e2e-status=$STATUS" >"$APP_LOG" 2>&1 &
APP_PID="$!"
