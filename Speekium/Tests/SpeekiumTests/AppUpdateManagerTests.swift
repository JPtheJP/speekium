...
final class AppUpdateManagerTests: XCTestCase {
    private let releasePage = URL(string: "https://github.com/JPtheJP/speekium/releases/tag/v1.0.2")!

    private func asset(_ name: String, size: Int64 = 100) -> GitHubRelease.Asset {
        GitHubRelease.Asset(
            name: name,
            downloadURL: URL(
                string: "https://github.com/JPtheJP/speekium/releases/download/v1.0.2/\(name)"
            )!,
            size: size
        )
    }
    ...
    func testDeveloperFeedAcceptsOnlyLoopbackHTTPAssetsWhenExplicitlyEnabled() throws {
        let archiveURL = URL(string: "http://127.0.0.1:8765/Speekium-macos-arm64.zip")!
        let signatureURL = URL(
            string: "http://localhost:8765/Speekium-macos-arm64.zip.ed25519"
        )!
    ...
    func testFetchLatestReleaseCanUseDeveloperFeed() async throws {
        ...
        {
          "tag_name": "v1.0.2",
          "html_url": "https://github.com/JPtheJP/speekium/releases/tag/v1.0.2",
          "draft": false,
          "prerelease": false,
          "assets": []
        }
    ...
    func testManagerDiscoversUpdateThroughDeveloperFeed() async throws {
        ...
        let archiveURL = URL(string: "http://127.0.0.1:8765/Speekium-macos-arm64.zip")!
        let signatureURL = URL(
            string: "http://127.0.0.1:8765/Speekium-macos-arm64.zip.ed25519"
        )!
        MockUpdateURLProtocol.responses = [feedURL: Data("""
        {
          "tag_name": "v1.0.2",
        ...
        let manager = AppUpdateManager(
            repository: "JPtheJP/speekium",
            currentVersion: "1.0.1",
            bundleIdentifier: "com.jpthejp.speekium",
            publicKey: key,
            appBundleURL: URL(fileURLWithPath: "/tmp/Speekium.app"),
            session: URLSession(configuration: configuration),
            developerFeedURL: feedURL
        )
    ...
    func testEd25519SignatureAcceptsExactArchiveAndRejectsTampering() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("SpeekiumUpdateTests-\(UUID().uuidString)")
    ...
    func testReplacementBundleMustKeepIdentityVersionKeyAndExecutable() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("SpeekiumBundleTests-\(UUID().uuidString)")
        let app = directory.appendingPathComponent("Speekium.app")
        ...
        let plist: [String: Any] = [
            "CFBundleIdentifier": "com.jpthejp.speekium",
            "CFBundleShortVersionString": "1.0.2",
            "CFBundleVersion": "3",
            "CFBundlePackageType": "APPL",
            "CFBundleExecutable": "Speekium",
            "LSMinimumSystemVersion": "14.0",
            "SpeekiumUpdatePublicKey": key,
        ]
        try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
            .write(to: contents.appendingPathComponent("Info.plist"))
        let executable = macOS.appendingPathComponent("Speekium")
        try Data("#!/bin/sh\nexit 0\n".utf8).write(to: executable)
        try FileManager.default.setAttributes([.posixPermissions: 0o700], ofItemAtPath: executable.path)

        XCTAssertNoThrow(try AppUpdateManager.validateReplacementBundle(
            at: app,
            expectedBundleIdentifier: "com.jpthejp.speekium",
    ...
    func testUpdateTrustRootEnvironmentOverridesAreDebugOnly() {
        let environment = [
            "SPEEKIUM_UPDATE_REPOSITORY": "attacker/repository",
            "SPEEKIUM_UPDATE_PUBLIC_KEY": "attacker-key",
        ]
        let repository = AppUpdateManager.configuredUpdateRepository(
            bundleValue: "JPtheJP/speekium",
...
#else
        XCTAssertEqual(repository, "JPtheJP/speekium")
    ...
    func testDownloadVerifiesExtractsAndValidatesReplacement() async throws {
        ...
        let directory = manager.temporaryDirectory
            .appendingPathComponent("SpeekiumDownloadTests-\(UUID().uuidString)")
        let source = directory.appendingPathComponent("source")
        let app = source.appendingPathComponent("Speekium.app")
        let contents = app.appendingPathComponent("Contents")
        let macOS = contents.appendingPathComponent("MacOS")
        ...
        let plist: [String: Any] = [
            ...
            "CFBundleExecutable": "Speekium",
            "LSMinimumSystemVersion": "14.0",
            "SpeekiumUpdatePublicKey": publicKey,
        ]
        try PropertyListSerialization.data(fromPropertyList: plist, format: .xml, options: 0)
            .write(to: contents.appendingPathComponent("Info.plist"))
        let executable = macOS.appendingPathComponent("Speekium")
        ...
        XCTAssertEqual(Bundle(url: prepared.app)?.bundleIdentifier, "com.jpthejp.speekium")
        XCTAssertEqual(
    ...
    func testQuarantineIsClearedOnlyFromPreparedBundleTree() throws {
        let manager = FileManager.default
        let directory = manager.temporaryDirectory
            .appendingPathComponent("SpeekiumQuarantineTests-\(UUID().uuidString)")
        let app = directory.appendingPathComponent("Speekium.app")
        let executable = app.appendingPathComponent("Contents/MacOS/Speekium")
