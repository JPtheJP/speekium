...
The script never modifies /Applications or the repository's Speekium.app.
Quit any running Speekium instance first. Press Control-C when finished.
EOF
    exit 2
}
...
TEMPLATE_APP="$REPOSITORY_ROOT/Speekium.app"
BUILD_DIR="$SCRIPT_DIR/.build/debug"
TEST_ROOT="$(mktemp -d /tmp/speekium-mock-update.XXXXXX)"
FEED_ROOT="$TEST_ROOT/feed"
INSTALL_ROOT="$TEST_ROOT/install"
TARGET_APP="$INSTALL_ROOT/Speekium.app"
REPLACEMENT_APP="$TEST_ROOT/replacement/Speekium.app"
ARCHIVE="$FEED_ROOT/Speekium-macos-arm64.zip"
SIGNATURE="$FEED_ROOT/Speekium-macos-arm64.zip.ed25519"
SERVER_PID=""
...
cleanup() {
    ...
    case "$TEST_ROOT" in
        /tmp/speekium-mock-update.*) rm -rf -- "$TEST_ROOT" ;;
    esac
}
trap cleanup EXIT INT TERM

[ -d "$TEMPLATE_APP" ] || {
    echo "error: build Speekium.app once before running the mock updater" >&2
    exit 1
}
if [ "$PREPARE_ONLY" = false ] && pgrep -x Speekium >/dev/null 2>&1; then
    echo "error: quit the currently running Speekium process first" >&2
    exit 1
fi

echo "Building current debug executables..."
swift build --package-path "$SCRIPT_DIR" >/dev/null
for executable in Speekium SpeekiumUpdateInstaller; do
...
cp "$BUILD_DIR/Speekium" "$TARGET_APP/Contents/MacOS/Speekium"
cp "$BUILD_DIR/Speekium" "$REPLACEMENT_APP/Contents/MacOS/Speekium"
cp "$BUILD_DIR/SpeekiumUpdateInstaller" \
    "$TARGET_APP/Contents/Helpers/SpeekiumUpdateInstaller"
cp "$BUILD_DIR/SpeekiumUpdateInstaller" \
    "$REPLACEMENT_APP/Contents/Helpers/SpeekiumUpdateInstaller"
...
configure_app() {
    ...
    /usr/libexec/PlistBuddy -c "Set :SpeekiumUpdatePublicKey $PUBLIC_KEY" "$plist"
...
payload = {
    "tag_name": "v9.9.9",
    "html_url": "https://github.com/JPtheJP/speekium/releases/tag/v9.9.9",
    "draft": False,
    "prerelease": False,
    "assets": [
        {
            "name": "Speekium-macos-arm64.zip",
            "browser_download_url": base_url + "/Speekium-macos-arm64.zip",
            "size": int(archive_bytes),
        },
        {
            "name": "Speekium-macos-arm64.zip.ed25519",
            "browser_download_url": base_url + "/Speekium-macos-arm64.zip.ed25519",
...
if [ "$PREPARE_ONLY" = true ]; then
    curl -fsS "$BASE_URL/Speekium-macos-arm64.zip" \
        -o "$TEST_ROOT/downloaded.zip"
    curl -fsS "$BASE_URL/Speekium-macos-arm64.zip.ed25519" \
...
SPEEKIUM_UPDATE_REPOSITORY=JPtheJP/speekium \
SPEEKIUM_UPDATE_PUBLIC_KEY="$PUBLIC_KEY" \
SPEEKIUM_UPDATE_FEED_URL="$BASE_URL/latest.json" \
    "$TARGET_APP/Contents/MacOS/Speekium" &
APP_PID=$!
